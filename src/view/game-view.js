...
export default function gameView() {
  ...
  function _placeShip(board, startLocation, length, direction) {
    ...
    if (direction === "horizontal") {
      ship.setAttribute(
        "style",
        `grid-area: ${y + 1} / ${x + 1} / span 1 / span ${length}`,
      );
    } else {
      ship.setAttribute(
        "style",
        `grid-area: ${y + 1} / ${x + 1} / span ${length} / span 1`,
      );
    }
  ...
  function _initializeBoard(board) {
    board.forEach((row, rowIndex) => {
      for (let colIndex = 0; colIndex < row.length; colIndex++) {
        ...
        button.setAttribute(
          "style",
          `grid-area: ${rowIndex + 1} / ${colIndex + 1}`,
        );
  ...
  function renderActivePlayer(activePlayer) {
    announcement.textContent = activePlayer.isComputer
  ...
  function renderCursorWaiting() {
    boards[1]
      .querySelector(".board-overlay")
      .classList.toggle("board-overlay--active");
  }

  function renderMiss(data) {
    const { boardId, location } = data;
    const board = [...boards].filter(
      (b) => b.getAttribute("id") === boardId,
    )[0];
    const miss = document.createElement("div");
    miss.classList.add("miss");
    miss.setAttribute(
      "style",
      `grid-area: ${location.y + 1} / ${location.x + 1}`,
    );

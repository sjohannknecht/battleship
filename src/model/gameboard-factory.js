...
const gameboardProto = {
  ...
  _canShipBePlaced(startLocation, length, direction) {
    ...
    if (
      this._isLocationOutOfBounds(startLocation) ||
      this._isLocationOutOfBounds({ x: x_end, y: y_end })
    ) {
      return false;
    }
  ...
  receiveAttack(location) {
    ...
    if (this.board[y][x] !== undefined && this.board[y][x] !== "miss") {
      ...
      if (this.board[y][x].ship.isSunk()) {
        messageBroker.publish("model:sunk", {
          boardId: this.id,
          ship: this.board[y][x].ship,
          location: this.getShipStartLocation(this.board[y][x].ship),
          direction: this.board[y][x].direction,
        });
      }
      return true;
    } else {
